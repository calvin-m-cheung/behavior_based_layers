...
namespace vfh_local_planner
{
    ...
    std::string odom_name = ns.erase(0,1)==""?"odom":ns.erase(0,1)+"_tf/odom";

    VFHPlannerRos::VFHPlannerRos(): costmap_ros_(NULL), tf_(NULL), initialized_(false), odom_helper_(odom_name) {};

    VFHPlannerRos::VFHPlannerRos(std::string name, tf::TransformListener* tf, costmap_2d::Costmap2DROS* costmap_ros)
        : costmap_ros_(NULL), tf_(NULL), initialized_(false), odom_helper_(odom_name)
    {
        // initialize planner
    ...
    void VFHPlannerRos::initialize(std::string name, tf::TransformListener* tf, costmap_2d::Costmap2DROS* costmap_ros)
    {
        ...
        if(!initialized_)
		{
            ...
            // Ready variables for publishing VFF Vector
            VFF_vector_pose_pub = private_nh.advertise<visualization_msgs::Marker>("VFF_vector",1);
        }
        else
        {
    ...
    bool VFHPlannerRos::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
    {
        ...
        ROS_INFO("Try to update Historgram");
        ...
        ROS_INFO("Check if the robot recahed the goal position");
        ...
        /*
         * if needing to find an alternate way, looks at global goals point. Otherwise, use an intermediate one. Note that the global goal point is the master list
         * and for intermediate, that list is trimmed down of thins that are too far away.
         */
        if(finding_alternative_way_)
        {
        	ROS_INFO("Finding alternate way");
        ...
        {
        	ROS_INFO("No alternate way needed");
            ...
            {
            	ROS_INFO("Direction is not clear");
        ...
        {
        	double drive_angle = angles::shortest_angular_distance(tf::getYaw(current_pose.getRotation()),direction_to_follow);
        	std::cout << "driving to goal" << std::endl;
            std::cout << "goal_distance:                          " << goal_distance << std::endl;
            std::cout << "drive_angle:                            " << drive_angle << std::endl;
            std::cout << "direction to follow:                    " << direction_to_follow << std::endl;
            std::cout << "cos(direction_to_follow)*goal distance: " << cos(direction_to_follow)*goal_distance<< std::endl;
            std::cout << "sin(direction_to_follow)*goal distance: " << sin(direction_to_follow)*goal_distance<< std::endl;
            vfh_planner.DriveToward(drive_angle, goal_distance, cmd_vel);



        }

        visualization_msgs::Marker VFF_vector_msg;
		VFF_vector_msg.header.frame_id = odom_name;
		VFF_vector_msg.header.stamp = ros::Time();
		VFF_vector_msg.ns = ros::this_node::getNamespace();
		VFF_vector_msg.id = 0;
		VFF_vector_msg.type = visualization_msgs::Marker::ARROW;
		VFF_vector_msg.action = visualization_msgs::Marker::ADD;
		VFF_vector_msg.scale.x = 0.1;
		VFF_vector_msg.scale.y = 0.1;
		VFF_vector_msg.scale.z = 0.1;
		VFF_vector_msg.color.a = 1.0; // Don't forget to set the alpha!
		VFF_vector_msg.color.r = 0.0;
		VFF_vector_msg.color.g = 1.0;
		VFF_vector_msg.color.b = 1.0;

		geometry_msgs::Point p;
		p.x= current_pose.getOrigin().x();
		p.y= current_pose.getOrigin().y();
		p.z= 0;
		VFF_vector_msg.points.push_back(p);
		p.x= current_pose.getOrigin().x()+cos(direction_to_follow)*goal_distance*10;
		p.y=current_pose.getOrigin().y()+sin(direction_to_follow)*goal_distance*10;
		p.z= 0;
		VFF_vector_msg.points.push_back(p);
		VFF_vector_pose_pub.publish(VFF_vector_msg);
        return true;
    };
