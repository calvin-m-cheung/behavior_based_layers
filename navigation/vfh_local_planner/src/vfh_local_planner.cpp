...
namespace vfh_local_planner
{
    ...
    bool VFHPlanner::DirectionIsClear(double goal_direction)
    {
        ...
        for (int k=0; k <= (config_.very_narrow_valley_threshold/2)-1; k++)
            {
        	/*
        	 * I think the ternary operators are to make sure the sectors loop around if you're around the 0/360 degree area. Look at both sides of the goal to make suure it's
        	 * at the center of a narrow valley.
        	 *
        	 * What this does overall is look at the goal, and keep expanding out on both sides to see if it meets the definition of narrow valley, by seeing if every angle that
        	 * would make up the narrow angel actually has a cost that's lower than the threshold. If they're every point is lower, means there's adequate (but narrow). If at any point
        	 * it's higher, that means that the "narrow valley" has an obstacle it can't get through.
        	 */
                ...
                if(lower_sector==vfh_histogram.size())
                	lower_sector--;

                std::cout << "vsec " << upper_sector << "  " << lower_sector << std::endl;
                std::cout << "vfh_histogram size: " << vfh_histogram.size() << std::endl;
                ROS_INFO("Histogram Upper Sector");
                bool test = vfh_histogram.at(upper_sector) > config_.vfh_threshold;
                ROS_INFO("Histogram Lower Sector");
                test = vfh_histogram.at(lower_sector) > config_.vfh_threshold;
                ROS_INFO("Out of lower sector");
                std::cout << "vfh_histogram.at(upper_sector) " << vfh_histogram.at(upper_sector) << std::endl;
                std::cout << "vfh_histogram.at(lower_sector) " << vfh_histogram.at(lower_sector) << std::endl;
